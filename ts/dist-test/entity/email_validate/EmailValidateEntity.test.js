...
(0, node_test_1.describe)('EmailValidateEntity', async () => {
    ...
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('DOMAIN_WHOIS_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.DomainWhoisSDK.test();
        const ent = testsdk.EmailValidate();
        (0, node_assert_1.default)(null != ent);
    });
    (0, node_test_1.test)('basic', async (t) => {
        const live = 'TRUE' === process.env.DOMAIN_WHOIS_TEST_LIVE;
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set DOMAIN_WHOIS_TEST_EMAIL_VALIDATE_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    let client = __1.DomainWhoisSDK.test(options, extra);
    ...
    const idmapEnvVal = process.env['DOMAIN_WHOIS_TEST_EMAIL_VALIDATE_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'DOMAIN_WHOIS_TEST_EMAIL_VALIDATE_ENTID': idmap,
        'DOMAIN_WHOIS_TEST_LIVE': 'FALSE',
        'DOMAIN_WHOIS_TEST_EXPLAIN': 'FALSE',
        'DOMAIN_WHOIS_APIKEY': '',
    });
    idmap = env['DOMAIN_WHOIS_TEST_EMAIL_VALIDATE_ENTID'];
    const live = 'TRUE' === env.DOMAIN_WHOIS_TEST_LIVE;
    if (live) {
        client = new __1.DomainWhoisSDK(merge([
    ...
    const setup = {
        ...
        struct,
        data: entityData,
        explain: 'TRUE' === env.DOMAIN_WHOIS_TEST_EXPLAIN,
        live,
