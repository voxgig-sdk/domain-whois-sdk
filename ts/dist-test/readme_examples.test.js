...
function isRunnable(code) {
    return (/new\s+DomainWhoisSDK\b/.test(code) ||
        /\bDomainWhoisSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+DomainWhoisSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/DomainWhoisSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
