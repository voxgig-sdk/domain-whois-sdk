# DomainWhois TypeScript SDK Reference

Complete API reference for the DomainWhois TypeScript SDK.
...
```ts
new DomainWhoisSDK(options?: object)
```
...
#### `DomainWhoisSDK.test(testopts?, sdkopts?)`
...
**Returns:** `DomainWhoisSDK` instance in test mode.
...
Return the parent `DomainWhoisSDK` instance.
