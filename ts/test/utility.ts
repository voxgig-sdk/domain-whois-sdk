...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.DOMAINWHOIS_TEST_LIVE ||
    'TRUE' === process.env.DOMAINWHOIS_TEST_OVERRIDE
  ) {
  ...
  m.DOMAINWHOIS_TEST_EXPLAIN = process.env.DOMAINWHOIS_TEST_EXPLAIN || m.DOMAINWHOIS_TEST_EXPLAIN
