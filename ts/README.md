# DomainWhois TypeScript SDK



The TypeScript SDK for the DomainWhois API — a type-safe, entity-oriented client with full async/await support.
...
- Releases: [https://github.com/voxgig-sdk/domain-whois-sdk/releases](https://github.com/voxgig-sdk/domain-whois-sdk/releases)
...
```ts
const client = DomainWhoisSDK.test()
...
```ts
const client = new DomainWhoisSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `DomainWhoisSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `DomainWhoisSDK.test(testopts?, sdkopts?)` | `DomainWhoisSDK` | Create a test-mode client. |
...
| `client` | `client(): DomainWhoisSDK` | Return the parent SDK client. |
...
domain-whois/
├── src/
│   ├── DomainWhoisSDK.ts        # Main SDK class
