...
class DomainWhoisTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DomainWhoisTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DomainWhoisTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DomainWhoisTestRunner.getenv("DOMAIN_WHOIS_TEST_LIVE")
        override = DomainWhoisTestRunner.getenv("DOMAIN_WHOIS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DomainWhoisTestRunner.getenv(key)
        ...
        explain = DomainWhoisTestRunner.getenv("DOMAIN_WHOIS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DOMAIN_WHOIS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DomainWhoisTestRunner._test_control is not None:
            return DomainWhoisTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DomainWhoisTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DomainWhoisTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DomainWhoisTestRunner.load_test_control()
...
def load_env_local():
    DomainWhoisTestRunner.load_env_local()


def env_override(m):
    return DomainWhoisTestRunner.env_override(m)


def entity_data(v):
    return DomainWhoisTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DomainWhoisTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DomainWhoisTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DomainWhoisTestRunner.load_test_control()


def live_client_options():
    return DomainWhoisTestRunner.live_client_options()


def live_delay_ms():
    return DomainWhoisTestRunner.live_delay_ms()
