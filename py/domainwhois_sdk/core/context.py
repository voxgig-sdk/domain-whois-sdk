...
from domainwhois_sdk.core.helpers import get_ctx_prop, to_map
...
class DomainWhoisContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, DomainWhoisSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, DomainWhoisResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, DomainWhoisResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return DomainWhoisError(code, msg, self)
