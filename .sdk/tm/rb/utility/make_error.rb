...
module DomainWhoisUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = DomainWhoisContext.new({}, nil)
    end
    op = ctx.op || DomainWhoisOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || DomainWhoisResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(DomainWhoisError) ? err.msg : err.to_s
    ...
    sdk_err = DomainWhoisError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(DomainWhoisError)
