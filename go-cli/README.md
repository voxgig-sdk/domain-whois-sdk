...
boru-driven command-line client **and** interactive REPL for the DomainWhois
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/domain-whois-cli)
make build

# 2. See usage (words, entities, env vars)
./domain-whois-cli --help

# 3. Provide credentials once, via the environment
export DOMAIN_WHOIS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./domain-whois-cli load 1 dns_result            # {id:1} shorthand
./domain-whois-cli load '{id:1}' dns_result       # explicit match map

# 5. Override the API base URL for a single call
DOMAIN_WHOIS_BASE=https://api.example.com ./domain-whois-cli load 1 dns_result

# 6. No arguments -> interactive REPL
./domain-whois-cli
domain-whois> load 1 dns_result
domain-whois> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/domain-whois-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/domain-whois-cli load 1 dns_result
   ```
...
```sh
./domain-whois-cli load 1 dns_result          # scalar shorthand for {id:1}
./domain-whois-cli load '{id:1}' dns_result     # explicit match map
```
...
./domain-whois-cli load 1 dns_result
```
...
Run with no arguments to open a REPL (prompt `domain-whois>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/domain-whois-cli`. |
...
(`./domain-whois-cli <expr>`) and interactively (the REPL), and expressions compose the
