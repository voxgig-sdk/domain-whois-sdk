// Command DomainWhois-cli is a CLI + REPL for the DomainWhois SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DOMAIN_WHOIS_APIKEY (api key), DOMAIN_WHOIS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DOMAIN_WHOIS_APIKEY carries the API key and
	// DOMAIN_WHOIS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DOMAIN_WHOIS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DOMAIN_WHOIS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDomainWhoisSDK(opts)
