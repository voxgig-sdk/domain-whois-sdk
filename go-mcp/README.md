...
[MCP](https://modelcontextprotocol.io) server exposing the DomainWhois SDK as
two agent tools — `domain-whois_list` and `domain-whois_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/domain-whois-mcp)
make build

# 2. Provide credentials via the environment
export DOMAIN_WHOIS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user domain-whois \
  -- /absolute/path/to/domain-whois-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./domain-whois-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// domain-whois_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// domain-whois_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/domain-whois-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user domain-whois \
     -- "$PWD"/dist/*/domain-whois-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `domain-whois_list` and `domain-whois_load` tools now appear
   in new sessions. Ask the agent to *"list domain using domain-whois"*
   and it calls `domain-whois_list` with `{"entity":"domain"}`.
...
```sh
export DOMAIN_WHOIS_APIKEY=sk_live_xxx            # API key
export DOMAIN_WHOIS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `domain-whois_list` tool
...
### Call the `domain-whois_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `domain-whois_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `domain-whois_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DOMAIN_WHOIS_APIKEY` | API key sent with every request. |
| `DOMAIN_WHOIS_BASE` | Optional override of the API base URL. |
...
```sh
./domain-whois-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"domain-whois_load","arguments":{"entity":"dns_result","query":{"id":1}}}}'
```
