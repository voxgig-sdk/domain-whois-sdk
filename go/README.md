# DomainWhois Golang SDK



The Golang SDK for the DomainWhois API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/domain-whois-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/domain-whois-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/domain-whois-sdk/go=../domain-whois-sdk/go
```
...
import (
    "fmt"
    "os"
    sdk "github.com/voxgig-sdk/domain-whois-sdk/go"
)

func main() {
    client := sdk.NewDomainWhoisSDK(map[string]any{
        "apikey": os.Getenv("DOMAIN_WHOIS_APIKEY"),
    })
...
```
DOMAIN_WHOIS_TEST_LIVE=TRUE
DOMAIN_WHOIS_APIKEY=<your-key>
```
...
```go
func NewDomainWhoisSDK(options map[string]any) *DomainWhoisSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *DomainWhoisSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### DomainWhoisSDK methods
...
| `DnsResult` | `(data map[string]any) DomainWhoisEntity` | Create a DnsResult entity instance. |
| `Domain` | `(data map[string]any) DomainWhoisEntity` | Create a Domain entity instance. |
| `EmailValidate` | `(data map[string]any) DomainWhoisEntity` | Create an EmailValidate entity instance. |
| `Generate` | `(data map[string]any) DomainWhoisEntity` | Create a Generate entity instance. |
| `Grammar` | `(data map[string]any) DomainWhoisEntity` | Create a Grammar entity instance. |
| `Ipn` | `(data map[string]any) DomainWhoisEntity` | Create an Ipn entity instance. |
| `Redact` | `(data map[string]any) DomainWhoisEntity` | Create a Redact entity instance. |
| `Ssl` | `(data map[string]any) DomainWhoisEntity` | Create a Ssl entity instance. |
| `Utility` | `(data map[string]any) DomainWhoisEntity` | Create an Utility entity instance. |
| `Whoi` | `(data map[string]any) DomainWhoisEntity` | Create a Whoi entity instance. |

### Entity interface (DomainWhoisEntity)

All entities implement the `DomainWhoisEntity` interface.
...
```
github.com/voxgig-sdk/domain-whois-sdk/go/
├── domain-whois.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/domain-whois-sdk/go`) re-exports everything needed
