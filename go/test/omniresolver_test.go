...
import (
	...
	omni "github.com/voxgig-sdk/domain-whois-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DomainWhoisError); is {
				out := map[string]any{
					"name":    "DomainWhoisError",
...
func omniCtx(arg any, client *sdk.DomainWhoisSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DomainWhoisSDK); is {
			client = live
