...
func TestDnsResultEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DOMAINWHOIS_TEST_DNS_RESULT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func dns_resultBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DOMAINWHOIS_TEST_DNS_RESULT_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"DOMAINWHOIS_TEST_DNS_RESULT_ENTID": idmap,
		"DOMAINWHOIS_TEST_LIVE":      "FALSE",
		"DOMAINWHOIS_TEST_EXPLAIN":   "FALSE",
		"DOMAINWHOIS_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["DOMAINWHOIS_TEST_DNS_RESULT_ENTID"])
