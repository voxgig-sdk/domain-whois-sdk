...
func TestRedactEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DOMAINWHOIS_TEST_REDACT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func redactBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DOMAINWHOIS_TEST_REDACT_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"DOMAINWHOIS_TEST_REDACT_ENTID": idmap,
		"DOMAINWHOIS_TEST_LIVE":      "FALSE",
		"DOMAINWHOIS_TEST_EXPLAIN":   "FALSE",
		"DOMAINWHOIS_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["DOMAINWHOIS_TEST_REDACT_ENTID"])
