...
require_once __DIR__ . '/../domainwhois_sdk.php';
...
{
    private static function utility(): DomainWhoisUtility
    {
        return new DomainWhoisUtility();
    }
    ...
    private static function ctx(array $over = []): DomainWhoisContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DomainWhoisContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DomainWhoisOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DomainWhoisError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DomainWhoisOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DomainWhoisMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DomainWhoisOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DomainWhoisMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DomainWhoisOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DomainWhoisMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DomainWhoisSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DomainWhoisMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DomainWhoisResponse([]),
            'result' => new DomainWhoisResult([]),
        ]);
        [, $err] = DomainWhoisMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DomainWhoisSpec([]),
            'response' => null,
            'result' => new DomainWhoisResult([]),
        ]);
        [, $err] = DomainWhoisMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DomainWhoisSpec(['step' => 's']),
            'response' => new DomainWhoisResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DomainWhoisResult([]),
        ]);
        [, $err] = DomainWhoisMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DomainWhoisSpec(['step' => 's']),
            'response' => new DomainWhoisResponse(self::resp(200, ['v' => 1])),
            'result' => new DomainWhoisResult([]),
        ]);
        [, $err] = DomainWhoisMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DomainWhoisSpec(['step' => 's']),
            'response' => new DomainWhoisResponse(self::resp(200, ['v' => 2])),
            'result' => new DomainWhoisResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DomainWhoisMakeResponse::call($ctx);
    ...
    {
        $preset = new DomainWhoisResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DomainWhoisSpec([]),
            'response' => new DomainWhoisResponse([]),
            'result' => new DomainWhoisResult([]),
        ...
        [$got, $err] = DomainWhoisMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DomainWhoisResult([])]);
        [, $err] = DomainWhoisMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DomainWhoisSpec([]), 'result' => null]);
        [, $err] = DomainWhoisMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DomainWhoisSpec(['step' => 's']),
            'result' => new DomainWhoisResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DomainWhoisOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DomainWhoisMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DomainWhoisSpec(['step' => 's']),
            'result' => new DomainWhoisResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DomainWhoisOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DomainWhoisMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DomainWhoisResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DomainWhoisSpec([]),
            'result' => new DomainWhoisResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DomainWhoisMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DomainWhoisMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DomainWhoisError('boom', 'boom');
        $utility->fetcher = function (DomainWhoisContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DomainWhoisSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DomainWhoisMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DomainWhoisContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DomainWhoisMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DomainWhoisResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        DomainWhoisMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DomainWhoisContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DomainWhoisSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DomainWhoisMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DomainWhoisResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DomainWhoisSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DomainWhoisMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DomainWhoisMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DomainWhoisMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DomainWhoisResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DomainWhoisDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DomainWhoisResult(['ok' => false])]);
        $this->expectException(DomainWhoisError::class);
        DomainWhoisDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DomainWhoisResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DomainWhoisMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DomainWhoisMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DomainWhoisBaseFeature();
        $b = new DomainWhoisBaseFeature();
        DomainWhoisFeatureAdd::call($ctx, $a);
        DomainWhoisFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DomainWhoisBaseFeature
    {
        $f = new DomainWhoisBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DomainWhoisFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DomainWhoisContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DomainWhoisSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DomainWhoisPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        DomainWhoisPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DomainWhoisResponse(['headers' => null]),
            'result' => new DomainWhoisResult([]),
        ]);
        DomainWhoisResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        DomainWhoisResultBody::call($ctx);
