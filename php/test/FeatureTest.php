...
class FtCtrl extends DomainWhoisControl
{
...
class FtClient
{
    ...
    private DomainWhoisUtility $utility;

    public function __construct(DomainWhoisUtility $utility, array $options)
    {
    ...
    public function get_utility(): DomainWhoisUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DomainWhoisContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DomainWhoisContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DomainWhoisContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DomainWhoisConfig::make_feature($name);
    ...
    private static function buildUrl(DomainWhoisSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DomainWhoisError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DomainWhoisResponse($fetched);
        }
    ...
    private function fail(DomainWhoisContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DomainWhoisContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
