...
class DomainEntityTest extends TestCase
{
    ...
    {
        $testsdk = DomainWhoisSDK::test(null, null);
    ...
    public function test_stream(): void
    {
        ...
        $base = DomainWhoisSDK::test($seed, null);
        $seen = iterator_to_array($base->Domain(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = DomainWhoisConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = DomainWhoisSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DOMAIN_WHOIS_TEST_DOMAIN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function domain_basic_setup($extra)
{
    ...
    $client = DomainWhoisSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("DOMAIN_WHOIS_TEST_DOMAIN_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["DOMAIN_WHOIS_TEST_DOMAIN_ENTID"]);
