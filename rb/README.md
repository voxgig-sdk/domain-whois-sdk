# DomainWhois Ruby SDK



The Ruby SDK for the DomainWhois API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
require_relative "DomainWhois_sdk"
client = DomainWhoisSDK.new(options)
```
...
```ruby
client = DomainWhoisSDK.test(testopts, sdkopts)
```
...
raise a `DomainWhoisError` (a `StandardError` subclass), so wrap
...
```
rb/
├── DomainWhois_sdk.rb       -- Main SDK module
...
The main module (`DomainWhois_sdk`) exports the SDK class
