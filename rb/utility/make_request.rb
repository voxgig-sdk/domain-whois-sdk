...
module DomainWhoisUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DomainWhoisResponse.new({})
    ...
    if fetch_err
      ...
      response = DomainWhoisResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DomainWhoisResponse.new(fetched)
    else
