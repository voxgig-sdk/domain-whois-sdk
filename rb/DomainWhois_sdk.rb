...
require_relative 'DomainWhois_types'
...
class DomainWhoisSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DomainWhoisHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DomainWhoisHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DomainWhoisFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    DomainWhoisUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DomainWhoisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DomainWhoisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DomainWhoisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = DomainWhoisHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DomainWhoisError.new(
        "graphql_error", "DomainWhoisSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = DomainWhoisSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
